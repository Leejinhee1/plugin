...
description: HES 컴포넌트 카탈로그를 조회하고, 새 컴포넌트를 추가하거나 기존 컴포넌트의 스펙·사용사례를 갱신합니다. 어떤 컴포넌트가 있는지, 특정 컴포넌트를 어떻게 쓰는지, 새 컴포넌트를 만들 때 사용. Use when listing components, authoring a new component (spec+code+usage), or updating usage examples.
---
...
3. `<Component>.tsx` — 레퍼런스 구현. **색/간격은 semantic 토큰 CSS 변수(`--hes-*`)만** 참조. 하드코딩 금지.
...
- 상세 검수는 `hes:design-reviewer` 에이전트에 위임 가능.
