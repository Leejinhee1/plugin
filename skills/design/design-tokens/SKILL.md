---
name: design-tokens
description: HES 디자인 토큰(색·타이포·간격·모션)을 조회·추가·수정합니다. 토큰 값을 바꾸거나, 새 semantic 토큰을 추가하거나, 특정 UI에 어떤 토큰을 써야 하는지 물을 때 사용. Use when adding/editing design tokens or choosing which token applies.
---
...
- HES 원본에 없는 값을 임의로 만들어 채우지 말 것(예: `space.elements.xl` 은 HES 에 없음). 자체 토큰을 추가할 땐 `$description` 에 "자체 토큰"을 명시.
- 새 토큰은 `category.role.variant` 명명. DTCG 포맷(`$value`/`$type`)을 지킨다.
- 색 추가 시 다크(`$dark`) 대응과 대비(WCAG AA)를 함께 확인한다 → 필요하면 `/hes:a11y-audit` 안내.
