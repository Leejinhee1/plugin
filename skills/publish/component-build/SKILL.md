...
2. **토큰 선행** — `/hds:token-export` 로 `tokens.css`(+tailwind preset)를 프로젝트에 설치/갱신하고, 엔트리에서 import 하도록 안내.
...
4. **검증** — 타입체크/빌드가 통과하는지 확인. `spec.md` 의 props·상태가 유지됐는지 대조. 접근성은 `/hds:a11y-audit`.
