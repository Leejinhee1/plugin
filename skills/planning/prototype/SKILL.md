...
- HDS 소스: `${CLAUDE_PLUGIN_ROOT}/design-system/` 의 `components/registry.json`, `usage/`, `tokens/`.

## 절차
1. 설계서의 화면 명세를 읽고, 참조된 컴포넌트를 `registry.json` 에서 확인한다.
2. 토큰 CSS 변수를 세팅한다: `/hds:token-export` 로 `tokens.css` 를 생성(또는 재사용)해 프로토타입에 로드.
3. 컴포넌트를 조립해 화면을 만든다:
...
5. 설계서에 없는 컴포넌트가 필요하면 임시 조립하되 **"신규 컴포넌트 후보"로 표시**하고 `/hds:component-catalog` 승격을 제안.
