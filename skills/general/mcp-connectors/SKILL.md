...
HES는 **파일 기반** 플러그인입니다 — 메인 플로우(기획→디자인→퍼블)는 커넥터 없이 전부 동작합니다. 다만 일부 스킬(`figma-bridge`·`brand-visual`)은 외부 MCP 서버를 쓰고, 그중 번들 가능한 것은 플러그인이 **직접 제공(등록)** 합니다. 이 스킬은 두 가지를 한 번에 보고합니다: (1) 플러그인이 **제공/인정하는 커넥터의 공식 목록**, (2) 지금 이 세션의 **실제 연결 상태**. 그리고 커넥터를 **등록·관리**합니다.
