 ...
 *
 * 기본값(인자 생략 시):
 *   --tokens  이 스크립트 기준 ../../hds-design/design-system/tokens (저장소 레이아웃 가정)
...
옵션:
  --tokens <dir>   core.tokens.json, semantic.tokens.json 이 있는 디렉터리
                   (기본값: ${relative(process.cwd(), resolve(SCRIPT_DIR, "../../hds-design/design-system/tokens"))})
...
function main() {
  ...
  const tokensDir = args.tokens
    ...
    : resolve(SCRIPT_DIR, "../../hds-design/design-system/tokens");
  ...
  if (!existsSync(tokensDir)) {
    console.error(
      `[build-tokens] 토큰 디렉터리를 찾을 수 없습니다: ${tokensDir}\n` +
        ...
        `  예) node build-tokens.mjs --tokens plugins/hds-design/design-system/tokens --out /tmp/out`
    );
    process.exit(1);
  }
