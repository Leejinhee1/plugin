...
2. **단일 플러그인, 역할별 스킬 버킷** — 플러그인은 `hes` 하나. 스킬은 `skills/planning|design|publish/` 로 분류하되 `plugin.json` 의 `skills` 배열에 명시적으로 나열합니다. 배열에 없는 폴더(초안 등)는 배포되지 않습니다.
...
/plugin install hes@hes                     # 이 한 번으로 기획·디자인·퍼블 전체 설치
```
개발 중에는 설치 없이:
```bash
...
검수는 어느 역할이든 `hes:design-reviewer` 에이전트로.
