...
**환경이 바뀌면**: `~/.hes/spec-source.json` 은 머신 단위라 새 환경(클라우드 세션·다른 PC)에는 없습니다. 팀·환경을 넘어 따라오게 하려면 프로젝트에 `.hes/spec-source.json` 을 **커밋**하세요. 깃 자격증명이 아예 없는 환경이라면 이미 있는 체크아웃을 `--local <경로>` 로 읽습니다 — 동기화를 하지 않으므로 스킬이 그 체크아웃의 커밋·시각을 답변에 밝힙니다.
