...
1. **처음 한 번만 대상 저장소를 알려줍니다** — "레포는 `<owner>/<name>` 이야" 라고 답하면 스킬이 `~/.hes/spec-source.json` 에 저장합니다(`--save-global`). **머신에 한 번이면 끝**이고, 그 뒤로는 어느 프로젝트에서 부르든 자동으로 잡힙니다. 프로젝트마다 다른 설계서를 본다면 프로젝트 `.hes/spec-source.json` 으로 덮어쓰면 됩니다.
...
**주의**: 설계서 저장소는 대개 **비공개**입니다. 읽기 권한이 없으면 스킬이 우회하지 않고 접근 요청을 안내합니다(`gh auth status` 로 계정 확인). 캐시는 설계서의 로컬 사본이므로 공유 머신에서는 사용 후 지우세요.

**환경이 바뀌면**: `~/.hes/spec-source.json` 은 머신 단위라 새 환경(클라우드 세션·다른 PC)에는 없습니다. 팀·환경을 넘어 따라오게 하려면 프로젝트에 `.hes/spec-source.json` 을 **커밋**하세요. 깃 자격증명 헬퍼가 없는 환경(컨테이너·CI·클라우드 세션)에서는 스킬이 `GH_TOKEN`·`GITHUB_TOKEN`·`gh auth token` 을 자동으로 찾아 씁니다 — 대개 `GH_TOKEN` 환경변수 하나면 됩니다(토큰은 디스크에 저장되지 않습니다). 토큰도 없고 체크아웃만 있다면 `--local <경로>` 로 읽되, 동기화를 하지 않으므로 스킬이 그 체크아웃의 커밋·시각을 답변에 밝힙니다.
