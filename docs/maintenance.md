...
2. **scope 결정**: `bundled`(자격증명 없는 로컬 stdio·공개 http만 — `plugin.json`/`.mcp.json` 선언) · `user`(각자 연결) · `project`(팀 `.mcp.json`). **비밀은 절대 커밋·번들 금지.**
...
- [ ] (MCP 커넥터 변경 시) `mcp/registry.json` 등록 + scope 적절 + **번들에 비밀 없음** + `url`엔 `type`?
