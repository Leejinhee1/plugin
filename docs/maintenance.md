...
2. 색이면 대비(WCAG AA)·다크(`$dark`) 확인 → `/hes:a11y-audit`.
3. `CHANGELOG.md` 기록 + version bump.
4. 소비처 재생성: `/hes:token-export`, (Figma 쓰면) `/hes:figma-bridge`.
> CSS 변수명이 컴포넌트 코드의 `--hes-*` 와 일치하는지 반드시 확인.

## 컴포넌트 추가/변경
1. `/hes:component-catalog` 로 스캐폴딩 → `components/<name>/` 에 3종 세트.
...
- `PATCH`: 값 조정·문서·버그.
- 릴리스 시: `.claude-plugin/plugin.json` 과 `marketplace.json` 의 version을 함께 bump하고 `CHANGELOG.md` 작성. 사용자는 `/plugin update hes@hes` 로 받는다.
