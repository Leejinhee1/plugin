...
## [3.10.0] — 2026-08-24
### 추가 — `spec-lookup` 이 자격증명 없는 환경에서도 동작한다 (토큰 인증 · `--local`)
클라우드 세션에서 부르니 **"설계서 저장소가 아직 지정되지 않았어요"** 로 멈췄습니다. 버전 문제가 아니라 설계의 구멍이었습니다 — 3.9.0 이 추가한 `~/.hes/spec-source.json` 은 **머신 단위**라 새 컨테이너엔 없고, 그 환경엔 깃 자격증명도 없어 비공개 레포 클론 자체가 불가능했습니다.

- **토큰 인증 추가** — 깃 자격증명 헬퍼가 없는 환경이 많습니다. `GH_TOKEN` → `GITHUB_TOKEN` → `gh auth token` 순으로 찾아 **그 호출에만** Authorization 헤더로 싣습니다(`git -c http.extraheader`). origin URL·`.git/config` 에 토큰이 남지 않는 것을 실측 확인했습니다. 자격증명이 전혀 없는 격리 환경에서 `GH_TOKEN` 하나로 클론·fetch 가 되는 것까지 검증.
- **`--filter=blob:none` 제거** — 이게 promisor remote 를 만들고, `checkout` 이 blob 을 **별도 프로세스에서** 지연 fetch 하면서 우리 토큰 헤더가 실리지 않아 자격증명 없는 환경에서 죽었습니다. `--depth 1` 만으로 충분히 작습니다(캐시 3.5MB · 1.2초).
- **`--local <경로>` 추가** — 이미 있는 체크아웃을 그대로 읽습니다. **클론도 동기화도 하지 않습니다**: 사용자의 체크아웃에 `fetch`·`pull`·`reset` 을 절대 걸지 않으므로 안전하지만 **최신 보장이 없고**, 그래서 그 체크아웃의 브랜치·커밋·시각·커밋 안 된 변경 여부를 출력하고 `"synced": false` 로 표시합니다. SKILL.md 는 이때 **답변에 "이 시점 기준"임을 반드시 밝히도록** 규정합니다. 설정 파일의 `localPath`·환경변수 `HES_SPEC_LOCAL` 로도 지정됩니다.
- **전제를 앞에 명시** — 이 스킬은 **쓰는 팀 자신의** 설계서 저장소를 읽는 빈 그릇이고, 플러그인은 어느 저장소도 기본값으로 갖지 않습니다. 설계서가 마크다운으로 Git 저장소에 있고 묻는 사람이 그 저장소를 읽을 수 있어야 한다는 전제를 SKILL.md 첫머리·`ask-hes`·README·스킬 표에 적었습니다. 전제가 없는 팀(Figma·PPT 기획서)에는 `test-script` 로 안내합니다.
- **"권한 없음"과 "환경에 자격증명 없음"을 갈랐습니다** — 권한이 없으면 **설정 문제가 아니라서** 토큰을 새로 만들어도 안 됩니다. 그 경우 저장소 관리자에게 접근을 요청하거나 `--repo` 로 볼 수 있는 저장소를 가리키라고 안내하고, "토큰을 설정하세요"로 헛수고시키지 않습니다.
- **"환경을 넘는 설정"을 안내** — `~/.hes` 가 머신 단위임을 명시하고, 팀·환경을 넘어 따라오게 하려면 **프로젝트에 `.hes/spec-source.json` 을 커밋**하라고 SKILL.md·getting-started·user-guide 에 적었습니다. 레포 이름은 비밀이 아니고, 그 프로젝트를 여는 모든 환경이 같은 설정을 갖습니다.
- **막혔을 때의 안내에 탈출구를 넣었습니다** — 미지정 오류와 권한 오류 양쪽에 `--local` 과 프로젝트 설정 커밋을 안내합니다. "안 된다"로 끝나지 않게.

## [3.9.1] — 2026-08-24
### 변경 — `spec-lookup` 답변이 규율을 낭독하지 않는다
실사용 답변이 문서 덤프처럼 나왔습니다 — 머리말·표를 층층이 쌓고, 끝에 "⚠️ 비공개 레포 본문입니다 — 외부로 옮기기 전 확인 필요", "규율상 실명·연락처는 옮기지 않습니다" 같은 **자기 규율 낭독**이 붙었습니다. 규율은 **지키는 것이지 말하는 것이 아니고**, 안 쓴 정보를 안 썼다고 알리는 것은 군더더기입니다.

- **"어떻게 말하나" 절 신설** — 문단으로 말하고, 표는 진짜 표일 때만(화면 인벤토리처럼 열이 있는 것). 근거는 본문을 끊지 말고 **맨 끝 한 줄**(경로·기준일·설계버전; 조회 커밋은 최신성이 쟁점일 때만). 모르는 것은 경고 블록이 아니라 흐름 안의 한 문장으로. 화면ID는 나열하지 말고 `기본모드(MAI_101)` 처럼 괄호로 거듭니다.
- **금지 문구를 명시** — "비공개 레포 본문입니다" · "외부로 옮기기 전 확인 필요" · "규율상 ~하지 않습니다". 외부 반출은 **실제로 그 행동을 할 때** 그 자리에서 확인합니다.
- 규율 자체는 그대로입니다 — 문서에 있는 것만, 없으면 없다고, 스텁과 부재를 구분, 개인정보는 답변에 옮기지 않음.

## [3.9.0] — 2026-08-24
### 변경 — `spec-lookup` 이 프로젝트마다 설정을 다시 묻지 않는다
3.8.0 은 대상 저장소를 프로젝트 `.hes/spec-source.json`·환경변수·인자에서만 찾았습니다. 원칙(추측 금지)은 맞지만 **레포는 사람마다 한 번 정하면 안 바뀌는데** 프로젝트를 옮길 때마다 처음부터 다시 물어, 답을 받으러 온 사람이 설정 안내부터 읽는 상황이 됐습니다. 해석 경로를 둘 늘려 첫 질문을 한 번으로 줄입니다.

- **홈 설정 `~/.hes/spec-source.json` 추가** — 프로젝트 설정 다음 순위. `--save-global` 로 한 번 저장하면 **어느 프로젝트에서 부르든** 자동으로 잡힙니다. 프로젝트마다 다른 설계서를 보는 경우에만 `--save`(프로젝트 `.hes/`)로 덮어씁니다.
- **캐시 폴백** — 지정이 하나도 없어도 이 머신의 캐시(`~/.cache/hes/spec-repos/`)에 레포가 **딱 하나**면 그것으로 조회하고, 출력의 `"source": "cache"` 와 경고로 **그 사실을 사용자에게 알리도록** 스킬에 명시했습니다. **후보가 둘 이상이면 고르지 않고 멈춥니다** — 추측 금지 원칙은 그대로입니다.
- **해석 순서**: `--repo` → `HES_SPEC_REPO` → 프로젝트 `.hes/` → 홈 `~/.hes/` → 캐시(유일할 때). 출력 JSON 에 `source` 를 실어 어디서 풀렸는지 보이게 했습니다.
- **못 풀었을 때의 안내를 짧게** — 선택지를 늘어놓지 말고 `owner/name` 한 줄만 요청하도록 SKILL.md 에 못박았습니다.
