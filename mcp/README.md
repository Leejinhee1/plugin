...
조회·현황 확인·등록·관리는 **`/hds:mcp-connectors`** 스킬로 합니다.

## bundled 의 진실은 plugin.json

`scope=bundled` 커넥터의 **실제 실행 config 는 `.claude-plugin/plugin.json` 의 `mcpServers` 가 진실**입니다. 이 레지스트리의 `connect.bundledConfig` 는 그 사본이며, 어긋나면 `plugin.json` 을 기준으로 삼습니다. 플러그인이 등록하므로 사용자는 서버를 따로 추가할 필요가 없고, 도구는 `mcp__plugin_hds_<서버명>__*` 로 노출됩니다.
