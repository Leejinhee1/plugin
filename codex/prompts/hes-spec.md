HES 제품 설계서(기획) 작업. 템플릿: `skills/planning/product-spec/templates/spec-template.md`.

요청($ARGUMENTS)에 따라 설계서를 작성/업데이트하라:

- 템플릿의 섹션·프론트매터 구조를 유지한다(AI-리더블).
- 화면은 자연어로만 쓰지 말고 HES 컴포넌트/토큰 이름으로 명세한다. 존재 여부는 `design-system/components/registry.json` 과 `usage/` 로 확인한다.
