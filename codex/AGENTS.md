# HES — Codex 하네스 규칙 (AGENTS.md)

이 저장소는 **HES 디자인 시스템**입니다. Codex CLI로 작업할 때 이 규칙을 따르세요.
...
## 불변 규칙
1. 색·간격·타이포·모션을 **하드코딩 금지**. 항상 semantic 토큰을 참조. (컴포넌트는 `--hes-*` CSS 변수 사용)
...
- `/hes-tokens`   — 토큰 조회/수정 (기획·디자인·퍼블 공통)
- `/hes-component` — 컴포넌트 조회/추가/코드화 (디자인·퍼블)
- `/hes-spec`     — 제품 설계서 작성/업데이트 (기획)

각 프롬프트는 위 소스 경로를 읽고 Claude 플러그인 스킬과 같은 규칙으로 동작합니다.

## Claude Code 플러그인과의 관계
Claude 사용자는 마켓플레이스에서 `hes` 플러그인 하나를 설치해 같은 콘텐츠를 스킬(`/hes:*`)로 사용합니다.
