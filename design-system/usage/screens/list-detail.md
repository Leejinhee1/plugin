...
주간 플랜의 일정 목록을 `Card` 리스트로 보여주고, 항목을 선택하면 `Modal`로 상세를 여는 패턴. "오늘 브리핑"의 일정 요약, 주간 플랜 화면에서 공통으로 쓴다.

## 목적

- 여러 일정을 한눈에 훑고, 상태(완료/예정/취소 등)를 즉시 구분할 수 있게 한다.
- 상세 확인이 필요할 때만 흐름을 잠깐 멈추는 가벼운 상호작용(Modal)으로 처리해, 목록 맥락을 잃지 않게 한다.
- 일정이 없을 때도 다음 행동을 제안해 화면이 막다른 길처럼 느껴지지 않게 한다(voice-tone `빈 상태` 톤).
...
- 리스트의 각 항목은 `Card`(`variant="default"`, `padding="md"`) 하나. 오늘 남은 일정 중 가장 임박한 1건만 `variant="elevated"`로 강조해도 좋다(elevated 남용 금지 — 화면당 1~2개).
- `CardHeader` 안에 일정 제목(`CardTitle as="h3"`)과 오른쪽 정렬된 `Badge`를 함께 배치.
  - `Badge` `variant`는 상태 의미에 맞춰 고정 매핑한다: 예정 `neutral`, 진행 중 `brand`, 완료 `success`, 지연/주의 `warning`, 취소/오류 `danger`.
  - `Badge` `size="sm"`(목록처럼 밀도가 높은 곳) 기본.
- `CardBody`에 시간(`오후 3:00`), 장소 등 보조 정보를 `color.fg.muted`로 표기.
...
```
Modal(open, onClose, title="일정 제목", size="md")
  본문: 시간 · 장소 · 메모 등 상세 필드 (space.stack-md 간격)
  footer: [Button variant=ghost "닫기"]  [Button variant=primary "일정 수정"]
```

- `title`은 일정 제목을 그대로 사용(필수 prop, `aria-labelledby` 자동 연결).
...
- 콘텐츠가 많아 목록형 상세(예: 하루 전체 일정)가 필요하면 `size="lg"`를 사용.
...
- **빈 상태(empty)**: 이번 주 등록된 일정이 없을 때, 목록 영역 전체를 하나의 안내 블록으로 대체한다.
  - 문구: "아직 등록된 일정이 없어요. 첫 일정을 추가해 볼까요?" (voice-tone 빈 상태 예시 그대로 적용)
  - 아래에 `Button`(`variant="primary"`) "일정 추가"로 다음 행동을 바로 제안한다.
...
- **오류(error)**: 목록을 불러오지 못하면 카드 대신 오류 안내 블록을 노출한다.
  - 문구: "일정을 불러오지 못했어요. 잠시 후 다시 시도해 주세요."
  - `Button`(`variant="secondary"`, "다시 시도")를 함께 배치. 전체 화면을 막는 파괴적 실패가 아니므로 `danger` variant는 쓰지 않는다.

## 접근성

- `CardHeader`를 감싸는 버튼에는 일정 제목과 상태를 함께 읽을 수 있도록, 시각적으로는 `Badge` 텍스트가 있으므로 추가 `aria-label` 없이도 스크린리더가 "일정 제목, 배지 텍스트" 순으로 읽는다 — 배지 텍스트가 항상 화면에 노출되어야 하는 이유이기도 하다(색만으로 상태 전달 금지).
