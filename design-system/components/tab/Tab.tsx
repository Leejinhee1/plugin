  ...
  return (
    <div
      ...
      className={cn(
        variant === "underline" &&
          cn(
            "flex border-b border-[var(--hds-border-base)]",
            scrollable && "overflow-x-auto [scrollbar-width:none]"
          ),
        variant === "round" &&
          cn("flex flex-wrap gap-2", isSticky && "sticky top-0 z-10"),
        variant === "toggle" &&
          "grid grid-cols-2 gap-px overflow-hidden rounded-[var(--hds-radius-sm)] border border-[var(--hds-border-base)] bg-[var(--hds-border-base)]",
        className
      )}
    >
      {items.map((item) => {
        ...
        const base =
          ...
          "focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-inset focus-visible:ring-[var(--hds-focus-ring)] " +
        ...
        return (
            ...
            className={cn(
              ...
              variant === "underline" &&
                cn(
                  ...
                  selected
                    ? "font-bold text-[var(--hds-fg-base)] after:absolute after:inset-x-0 after:bottom-0 after:h-0.5 after:bg-[var(--hds-fg-base)]"
                    : "text-[var(--hds-fg-muted)]"
                ),
              variant === "round" &&
                cn(
                  "rounded-[var(--hds-radius-full)] px-4 py-2 text-sm whitespace-nowrap",
                  selected
                    ? "bg-[var(--hds-brand-default)] font-bold text-[var(--hds-fg-on-brand)]"
                    : cn(
                        "text-[var(--hds-fg-muted)]",
                        // sticky 로 뜰 때 미선택 필은 배경 없음 (HES 규칙)
                        isSticky ? "bg-transparent" : "bg-[var(--hds-bg-subtle)]"
                      )
                ),
              variant === "toggle" &&
                cn(
                  ...
                  selected
                    ? "bg-[var(--hds-bg-base)] font-bold text-[var(--hds-fg-base)] outline outline-1 -outline-offset-1 outline-[var(--hds-border-strong)]"
          ...
          >
            ...
            {variant === "toggle" && item.subLabel && (
              <span className="mt-0.5 text-xs font-normal text-[var(--hds-fg-muted)]">
                {item.subLabel}
              </span>
            )}
          </button>
