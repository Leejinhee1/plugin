...
a11y-audit 스킬(🚧 준비중)로 자동 검증할 예정입니다. **지금은** 이 표를 기준으로 수동 검토하거나 `hes:design-reviewer` 에이전트에 검수를 맡기세요.
...
## 다크 테마
`semantic.tokens.json` 의 `$dark` 블록이 오버라이드를 정의합니다. 컴포넌트는 테마를 몰라도 되며, `[data-theme=dark]` 스코프에서 CSS 변수만 교체됩니다. HES Figma의 Dark 모드는 현재 Light와 사실상 동일하게 정의되어 있어, 다크 값은 자체적으로 grayScale 램프 반전으로 정의합니다(HES 다크 모드가 실제 값을 갖게 되면 figma-bridge로 재동기화).
